				...
				<div class="row">
					<div class="col-xs-12">
						...
						<form id="formSubmit">
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" id="hinhanh"
										name="hinhanh" value="${model.hinhanh}" />
								</div>
							</div>
							<br /> <br />
							<div class="form-group">
								...
								<div class="col-sm-9">
									<textarea rows="" cols="" id="diachi" name="diachi"
										style="width: 820px; height: 175px">${model.diachi}</textarea>
								</div>
							</div>
							<br /> <br />
							<div class="form-group">
								<div class="col-sm-12">
									<c:if test="${not empty model.idPet}">
										<input type="button"
											class="btn btn-white btn-warning btn-bold"
											value="Cập nhật bài viết" id="btnAddOrUpdateNew" />
									</c:if>
									<c:if test="${empty model.idPet}">
										<input type="button"
											class="btn btn-white btn-warning btn-bold"
							...
							<input type="hidden" value="${model.idPet}" id="idPet" name="idPet" />
						</form>
					</div>
				</div>
			</div>
			...
			$
					.ajax({
						...
						success : function(result) {
							window.location.href = "${NewURL}?type=edit&idPet="
									+ result.idPet + "&message=insert_success";
						},
		...
		function updateNew(data) {
			$
					.ajax({
						...
						success : function(result) {
							window.location.href = "${NewURL}?type=edit&idPet="
									+ result.idPet + "&message=update_success";
						},
						error : function(error) {
							window.location.href = "${NewURL}?type=list&maxPageItem=2&page=1&message=error_system";
						}
